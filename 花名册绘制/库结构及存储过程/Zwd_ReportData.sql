...
2-分组的情况
	...
	4-处理平均人数、累计、累计平均
**************************************************************************************************/
ALTER                 PROCEDURE Zwd_ReportData
	(
...
--execute Zwd_ErrorMessage @InfoFields, "Zwd_ReportData Object"

				if(@InfoFields = '12:AB0110:AB01.ZA0101')
					set @tempIndex = 9999
				--Modify 2004.6.15 FW
				--if(@InfoFields = '12:AB0111:AB01.ZA0101')
				--set @tempIndex = 8888
				--end moidfy

				execute Zwd_ParseField @InfoId,@Object,@Object,@InfoFields,"","","","1","",0,@DataType output,"",@strTemp output,@strFrom output,@strWhere output,"","","",@GroupBy2 output,@tempIndex
			...
			-- end add
				...
				if(@GetOrgLevel = 0)
					...
					begin
					   ...
					   --2004.6.15 fw 添加一个level  
			                   --set @strWhere = "AB01.level1 = ((select max(level1) from AB01)-"+ cast(@GetOrgLevel as varchar(1))+")"
					   --end level
					end
...
--execute Zwd_ErrorMessage @strTemp, "Zwd_ReportData"
		...
		-- add by zhangwd 2004-06-04
		if(charindex('T9999', @strFrom) > 0)
		begin
--execute Zwd_ErrorMessage @strFrom, "Zwd_ReportData strFrom"
--execute Zwd_ErrorMessage @strSelect, "Zwd_ReportData strSelect"

			set @strFrom = replace(@strFrom, 'LEFT JOIN AB01 ON AA01.AB0110=AB01.ZA0100', '')
			set @strFrom = replace(@strFrom, 'T99990', 'AB01')
			set @strSelect = replace(@strSelect, 'T99990', 'AB01')
			
		end
		-- end add	

		--add 2004.6.15 FW
		set @strSelect = replace(@strSelect, ')'+char(39)+'+'+char(39)+'(', ',')


		if(charindex('LEFT JOIN AB01 AS AB01 ON', @strFrom) > 0)
		begin
			set @strFrom = replace(@strFrom, 'LEFT JOIN AB01 ON AA01.AB0110=AB01.ZA0100', '')
			set @strFrom = replace(@strFrom, 'LEFT JOIN AB01 ON AA01.AB0111=AB01.ZA0100', '')
		end
	        --end add	
